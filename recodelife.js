...
function pageLoad() {
  // This function is a container function for ordering the processes.
  ...
  console.log(
    `Page dimensions set from viewport generated dimensions: Height: ${dimensionsArrayHeight}, Width: ${dimensionsArrayWidth}`
  );
  // Log for testing.
...
function appendToNavBar(navBarId, newPara, dimensions) {
  ...
  navBarId = document.getElementById('displayUpdate'); //This now points to the h4 placeholder element in the index file
  //newPara = document.createElement('p'); This line is no longer needed
  ...
  //newPara.setAttribute('id', 'displayUpdate'); The id of the element is now set in the index file
  if (beingResized) {
    navBarId.innerHTML = `Current ViewPort Dimensions: Max Height - ${dimensions[0]}, Max Width - ${dimensions[1]}`;
    //navBarId.append(newPara); This line is no longer needed
  }
}
